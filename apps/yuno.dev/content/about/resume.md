...
관심사 기반 SNS Vingle에서 커리어를 시작해 SaaS 노코드 솔루션을 만들었고, 현재는 캐치테이블에서 B2B 프론트엔드를 개발하고 있습니다.
...
- 관리자 페이지에서 블럭킷(서버드리븐 UI) 프리뷰가 필요했으나, 렌더링 컴포넌트가 B2C에만 존재하여 별도 페이지를 열어야 하는 불편함이 있었습니다. 이를 해결하기 위해 실 서비스 중인 관리자/B2C를 모노레포로 **단독으로** 작업해 이전했습니다.
...
라이브러리의 내부 구현을 직접 뜯어보고 이해하는 과정을 즐깁니다.

관심 있는 기술이 생기면 단순히 사용하는 데서 그치지 않고,  
아이디어를 얻어 새로운 라이브러리를 만들어 보거나  
이해를 위해 기존 라이브러리를 클론 코딩해 나만의 형태로 재구성하며 학습하고 있습니다.
